...
class PwaController extends Controller
{
    public function manifest(array $params = array())
    {
        ...
        $manifest = array(
            'name'             => $cfg['name'],
            'short_name'       => mb_substr($cfg['name'], 0, 12),
            'description'      => $cfg['tagline'] !== '' ? $cfg['tagline'] : 'Menú digital de ' . $cfg['name'],
            'id'               => $base,
            'start_url'        => $base,
            'scope'            => $base,
            'display'          => 'standalone',
    ...
    /** Iconos propios si el dueño ya subió su logo; si no, los de fábrica. */
    private function icons()
    {
        $dir = MG_ROOT . '/uploads/icons';
        $prefijo = '/uploads/icons';
        if (!is_dir($dir) || !is_file($dir . '/icon-512.png')) {
            $dir = MG_ROOT . '/assets/icons';
            $prefijo = '/assets/icons';
        }
        $icons = array();
        foreach (array(72, 96, 128, 144, 152, 192, 384, 512) as $s) {
            if (!is_file($dir . '/icon-' . $s . '.png')) { continue; }
            $icons[] = array(
                'src'     => Url::to($prefijo . '/icon-' . $s . '.png'),
                'sizes'   => $s . 'x' . $s,
                'type'    => 'image/png',
