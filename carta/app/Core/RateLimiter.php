...
final class RateLimiter
{
    ...
    public static function attempt($bucket, $maxHits, $windowSeconds, $identifier = null)
    {
        ...
        $now = time();
        try {
            $row = DB::first('SELECT id, hits, window_start FROM mg_rate_limits WHERE bucket_key = :k LIMIT 1', array('k' => $key));
            if (!$row) {
                DB::insert('mg_rate_limits', array('bucket_key' => $key, 'hits' => 1, 'window_start' => $now));
                return true;
            }
            if ($now - (int)$row['window_start'] >= $windowSeconds) {
                DB::update('mg_rate_limits', array('hits' => 1, 'window_start' => $now), 'id = :id', array('id' => (int)$row['id']));
                return true;
            }
            if ((int)$row['hits'] >= $maxHits) {
                return false;
            }
            DB::update('mg_rate_limits', array('hits' => (int)$row['hits'] + 1), 'id = :id', array('id' => (int)$row['id']));
            return true;
        } catch (\Throwable $e) {
    ...
    public static function retryAfter($bucket, $windowSeconds, $identifier = null)
    {
        $id = $identifier !== null ? $identifier : Security::clientIp();
        $key = substr($bucket . '|' . $id, 0, 190);
        $start = (int)DB::value('SELECT window_start FROM mg_rate_limits WHERE bucket_key = :k', array('k' => $key), 0);
    ...
    public static function clear($bucket, $identifier = null)
    {
        $id = $identifier !== null ? $identifier : Security::clientIp();
        $key = substr($bucket . '|' . $id, 0, 190);
        try { DB::delete('mg_rate_limits', 'bucket_key = :k', array('k' => $key)); } catch (\Throwable $e) {}
    }

    public static function prune()
    {
        try { DB::run('DELETE FROM mg_rate_limits WHERE window_start < :t', array('t' => time() - 86400)); } catch (\Throwable $e) {}
    }
}
