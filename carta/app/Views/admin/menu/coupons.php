...
<div class="grid grid-side">
          ...
          <tbody>
            <?php foreach ($coupons as $c): ?>
              <tr>
                ...
                <td class="num tabular"><?= (int)$c['used_count'] ?><?= (int)$c['max_uses'] > 0 ? ' / ' . (int)$c['max_uses'] : '' ?></td>
                <td class="muted" style="font-size:12px"><?= e(($c['starts_at'] ? mg_date($c['starts_at'], 'd/m/y') : '—') . ' → ' . ($c['ends_at'] ? mg_date($c['ends_at'], 'd/m/y') : '—')) ?></td>
                <td class="num">
                  <form method="post" action="<?= e(mg_url('/panel/menu/cupon/' . (int)$c['id'] . '/eliminar')) ?>" data-confirm="¿Eliminar el cupón <?= e($c['code']) ?>?">
  ...
  <form class="card" method="post" action="<?= e(mg_url('/panel/menu/cupones')) ?>">
    <?= Csrf::field() ?>
