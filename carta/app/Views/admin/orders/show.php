...
$cur = $cfg['currency'];
$open = !in_array($o['status'], array('closed', 'cancelled'), true);
?>
<?php $view->start('actions') ?>
  <a class="btn btn-sm btn-ghost" href="<?= e(mg_url('/panel/pedido/' . (int)$o['id'] . '/ticket')) ?>" target="_blank" rel="noopener">Ticket</a>
  <a class="btn btn-sm btn-ghost" href="<?= e(mg_url('/panel/pedido/' . (int)$o['id'] . '/ticket?formato=pdf')) ?>" target="_blank" rel="noopener">PDF</a>
<?php $view->stop() ?>
  ...
  <div class="stack">
    <div class="card">
      <div class="card-head">
        ...
        <span class="chip <?= $o['status'] === 'closed' ? 'chip-green' : ($o['status'] === 'cancelled' ? 'chip-ember' : '') ?>"><?= e($o['status_label']) ?></span>
      </div>

      <?php foreach ($o['items'] as $it): ?>
        <div class="row-between" style="padding:.8rem 0;border-bottom:1px solid var(--line-soft);align-items:flex-start">
          <div>
            <b class="cell-title"><?= (int)$it['qty'] ?>× <?= e($it['name']) ?></b>
    ...
    <div class="card">
      ...
      <ul class="stack" style="gap:.6rem;font-size:var(--step--1)">
        <li class="row-between"><span>Recibido</span><span class="faint" style="font-size:12px"><?= e(mg_date($o['placed_at'], 'd/m H:i')) ?></span></li>
        <?php if (!empty($o['ready_at'])): ?>
          <li class="row-between"><span>Listo en cocina</span><span class="faint" style="font-size:12px"><?= e(mg_date($o['ready_at'], 'd/m H:i')) ?></span></li>
        <?php endif; ?>
        <?php if (!empty($o['closed_at'])): ?>
          <li class="row-between"><span>Cobrado</span><span class="faint" style="font-size:12px"><?= e(mg_date($o['closed_at'], 'd/m H:i')) ?></span></li>
        <?php endif; ?>
        <?php if ($o['cancel_reason'] !== ''): ?>
          <li class="row-between"><span>Anulado</span><span class="muted"><?= e($o['cancel_reason']) ?></span></li>
        <?php endif; ?>
      </ul>
    </div>
  </div>
  ...
  <div class="stack">
    <?php if ($o['customer_name'] !== '' || $o['customer_phone'] !== '' || $o['address'] !== ''): ?>
      <div class="card">
        ...
        <?php if ($o['customer_phone'] !== ''): ?>
          ...
          <p class="mt-1"><a class="link-line gold" href="<?= e(mg_wa($o['customer_phone'], 'Hola, le escribimos de ' . $cfg['name'] . ' por su pedido ' . $o['code'] . '.')) ?>" target="_blank" rel="noopener">Escribir por WhatsApp</a></p>
        <?php endif; ?>
    ...
    <?php if ($open): ?>
      <div class="card">
        ...
        <div class="row">
          <?php
          foreach (array('cooking' => 'Preparando', 'ready' => 'Listo', 'served' => 'Entregado') as $st => $label):
            if ($st === $o['status']) { continue; }
          ?>
            <form method="post" action="<?= e(mg_url('/panel/pedido/' . (int)$o['id'] . '/estado')) ?>">
              <?= Csrf::field() ?>
      ...
      <form class="card" method="post" action="<?= e(mg_url('/panel/pedido/' . (int)$o['id'] . '/cobrar')) ?>">
        <?= Csrf::field() ?>
        ...
        <div class="field"><label for="method">Forma de pago</label>
          <select class="select" id="method" name="method">
            <?php foreach (MenuGold\Models\Settings::list('payment_methods') as $pm): ?>
              <option value="<?= e($pm) ?>"><?= e(ucfirst($pm)) ?></option>
            <?php endforeach; ?>
          </select></div>
        <div class="field"><label for="discount_percent">Descuento (%)</label>
          <input class="input" id="discount_percent" name="discount_percent" type="number" min="0" max="100" step="1" value="0"></div>
      ...
      <form class="card" method="post" action="<?= e(mg_url('/panel/pedido/' . (int)$o['id'] . '/estado')) ?>"
    ...
    <?php else: ?>
      <div class="card">
        ...
        <p class="muted" style="font-size:var(--step--1)">
          <?= $o['status'] === 'closed'
            ? 'Cobrado el ' . e(mg_date($o['closed_at'])) . ' en ' . e($o['payment_method']) . '.'
