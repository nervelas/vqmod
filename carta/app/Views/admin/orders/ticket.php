...
<div class="actions">
  ...
  <a href="<?= e(mg_url('/panel/pedido/' . (int)$o['id'] . '/ticket?ancho=' . ($w === 80 ? 58 : 80) . ($isPre ? '&precuenta=1' : ''))) ?>"><?= $w === 80 ? '58 mm' : '80 mm' ?></a>
  <a href="<?= e(mg_url('/panel/pedido/' . (int)$o['id'] . '/ticket?formato=pdf&ancho=' . $w . ($isPre ? '&precuenta=1' : ''))) ?>">PDF</a>
  <a href="<?= e(mg_url('/panel/pedido/' . (int)$o['id'])) ?>">Volver</a>
</div>

<div class="paper">
  <h1><?= e(mb_strtoupper($cfg['name'])) ?></h1>
  <?php if ($cfg['address'] !== ''): ?><p class="sub"><?= e($cfg['address']) ?></p><?php endif; ?>
  <?php if ($cfg['phone'] !== ''): ?><p class="sub">Tel. <?= e($cfg['phone']) ?></p><?php endif; ?>
  <hr>
  ...
  <?php foreach ($o['items'] as $it): ?>
    <div class="item">
      <div class="row"><span><?= (int)$it['qty'] ?> x <?= e($it['name']) ?></span><span><?= e(mg_money($it['line_total'], $cur)) ?></span></div>
  ...
  <?php if ($cfg['review_url'] !== ''): ?>
    <div class="qr">
      <img src="<?= e(\MenuGold\Core\Qr::dataUri($cfg['review_url'], 4)) ?>" alt="Código QR para dejar una reseña">
