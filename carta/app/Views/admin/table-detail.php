...
$cur = $cfg['currency'];
$tipOptions = array_values(array_filter(array_map('intval', explode(',', $cfg['tip_options']))));
?>
  ...
  <div class="grid grid-side">
    <div class="stack">
      <?php foreach ($orders as $o): ?>
        <div class="card">
          <div class="card-head">
            <div>
              <h3><a class="link-line" href="<?= e(mg_url('/panel/pedido/' . (int)$o['id'])) ?>"><?= e($o['code']) ?></a></h3>
          ...
          <ul class="stack" style="gap:.35rem;font-size:var(--step--1)">
            <?php foreach ($o['items'] as $it): ?>
              <li class="row-between">
                <span><?= (int)$it['qty'] ?>× <?= e($it['name']) ?></span>
    ...
    <div class="stack">
      ...
      <div class="card">
        ...
        <div class="row mt-1">
          <?php foreach ($orders as $o): ?>
            <a class="btn btn-ghost btn-sm" href="<?= e(mg_url('/panel/pedido/' . (int)$o['id'] . '/ticket?precuenta=1')) ?>" target="_blank" rel="noopener"><?= e($o['code']) ?></a>
          <?php endforeach; ?>
        </div>
      </div>
    </div>
