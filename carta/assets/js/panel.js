  ...
  function initKitchen() {
    ...
    function paintTimers() {
      els('.ticket[data-placed]').forEach(function (t) {
        ...
        var tarde = Number(cfg.lateMin || 18);
        t.classList.toggle('warn', mins >= Math.max(4, tarde - 8) && mins < tarde);
        t.classList.toggle('late', mins >= tarde);
      });
    }
    ...
    board.addEventListener('click', function (e) {
      ...
      if (!btn) { return; }
      btn.disabled = true;
      post(cfg.base + '/panel/pedido/' + btn.dataset.order + '/estado', { status: btn.dataset.status })
        .then(function (r) {
