<?php
/**
 * CorreoRadar - Motor de extracción de correos.
 *
...
final class Extractor
{
    ...
    private function desdeAtributos(string $html): void
    {
        ...
        foreach ($m as $par) {
            ...
            // Solo merece la pena si hay algo que pueda ser un correo.
            if (!preg_match('~[@]|&#|%40|\[at\]|\(at\)|arroba|=~i', $valor)) { continue; }
    ...
    private function recolectarTelefonos(string $html): void
    {
        ...
        $patrones = [
            '~(?:api|web|chat)\.whatsapp\.com/send/?\?[^"\'\s<>]*?phone=%?2?B?\+?(\d{7,15})~i',
            '~whatsapp://send/?\?[^"\'\s<>]*?phone=%?2?B?\+?(\d{7,15})~i',
            '~["\'](?:https?://)?(?:api|web)\.whatsapp\.com/send\?phone=(\d{7,15})~i',
        ];
        foreach ($patrones as $re) {
            if (preg_match_all($re, $html, $ma)) {
                foreach ($ma[1] as $n) { $this->anotarTelefono($n, 'api-whatsapp', true, true, $html); }
            }
        }
        ...
        if (preg_match_all('~data-(?:phone|tel|telefono|number|whatsapp|wa)\s*=\s*["\']([^"\']{7,30})["\']~i', $html, $md)) {
            foreach ($md[1] as $v) { $this->anotarTelefono($v, 'atributo', false, false, $v); }
        }
    }
