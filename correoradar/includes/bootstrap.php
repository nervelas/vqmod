<?php
/**
 * CorreoRadar - Arranque de la aplicación.
 *
...
if (version_compare(PHP_VERSION, CR_PHP_MINIMO, '<')) {
    ...
    exit('CorreoRadar necesita PHP ' . CR_PHP_MINIMO . ' o superior. Este servidor usa PHP ' . PHP_VERSION . '.');
}
...
if (!is_file($cr_config)) {
    ...
    if (basename($_SERVER['SCRIPT_NAME'] ?? '') !== 'install.php') {
        if (str_contains($_SERVER['SCRIPT_NAME'] ?? '', '/api/')) {
            ...
            exit(json_encode(['ok' => false, 'error' => 'CorreoRadar todavia no esta instalado.']));
        }
