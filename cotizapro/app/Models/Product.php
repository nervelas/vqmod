...
final class Product
{
    ...
    {
        return DB::one('SELECT * FROM products WHERE id = ? LIMIT 1', [$id]);
    }
    ...
    {
        return DB::one('SELECT * FROM products WHERE code = ? LIMIT 1', [$code]);
    }
    ...
    {
        return DB::one('SELECT * FROM product_images WHERE product_id = ? ORDER BY sort, id LIMIT 1', [$productId]);
    }

    public static function documents(int $productId): array
    {
        return DB::all('SELECT * FROM product_documents WHERE product_id = ? ORDER BY id', [$productId]);
    }
    ...
    public static function attributes(int $productId): array
    {
        return DB::all(
            'SELECT pa.value, a.code, a.label, a.unit, a.type, a.sort
             ...
             JOIN attribute_defs a ON a.id = pa.attribute_id
             WHERE pa.product_id = ?
    ...
    public static function attachImages(array $products): array
    {
        if (!$products) {
            return $products;
        }
        ...
        $rows = DB::all(
            "SELECT product_id, path, path_webp, path_thumb, alt, blur, width, height
             FROM product_images WHERE product_id IN ($in) ORDER BY sort DESC, id DESC",
            $ids
        );
        $map = [];
        foreach ($rows as $r) {
    ...
    public static function search(array $f = []): array
    {
        ...
        foreach ((array) ($f['attr'] ?? []) as $code => $val) {
            ...
            $n++;
            $joins .= " JOIN product_attributes pa{$n} ON pa{$n}.product_id = p.id"
                    . " JOIN attribute_defs ad{$n} ON ad{$n}.id = pa{$n}.attribute_id AND ad{$n}.code = ?";
            $joinParams[] = $code;
            $where[] = "pa{$n}.value = ?";
            $whereParams[] = $val;
        }
        ...
        if (!empty($f['category_id'])) {
            $ids = Category::descendantIds((int) $f['category_id']);
        ...
        $rows = DB::all(
            "SELECT DISTINCT p.id, p.category_id, p.brand_id, p.code, p.name, p.slug,
        ...
        return [self::attachImages($rows), $total];
    }
    ...
    public static function related(array $product, int $limit = 4): array
    {
        $rows = DB::all(
            ...
            [(int) $product['id'], (int) ($product['category_id'] ?? 0)]
        );
        if (count($rows) < $limit) {
            $extra = DB::all(
                'SELECT p.* FROM products p WHERE p.active = 1 AND p.id <> ?
    ...
    public static function uniqueSlug(string $base, ?int $ignoreId = null): string
    {
        ...
        $i = 1;
        while (true) {
            $sql = 'SELECT id FROM products WHERE slug = ?' . ($ignoreId ? ' AND id <> ?' : '') . ' LIMIT 1';
    ...
    public static function uniqueCode(string $base, ?int $ignoreId = null): string
    {
        $code = mb_substr(trim($base), 0, 55);
        $i = 1;
        while (true) {
            $sql = 'SELECT id FROM products WHERE code = ?' . ($ignoreId ? ' AND id <> ?' : '') . ' LIMIT 1';
            $p = $ignoreId ? [$code, $ignoreId] : [$code];
            if (!DB::one($sql, $p)) {
                return $code;
            }
    ...
    public static function priceFor(int $productId, ?int $priceListId): float
    {
        $base = (float) DB::value('SELECT price FROM products WHERE id = ?', [$productId], 0);
        if (!$priceListId) {
            return $base;
        }
        $custom = DB::value('SELECT price FROM product_prices WHERE product_id = ? AND price_list_id = ?', [$productId, $priceListId]);
        if ($custom !== null && (float) $custom > 0) {
            return (float) $custom;
        }
        $pct = (float) DB::value('SELECT discount_pct FROM price_lists WHERE id = ?', [$priceListId], 0);
