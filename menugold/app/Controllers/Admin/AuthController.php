...
class AuthController extends Controller
{
    public function login()
    {
        ...
        if (!$this->request->isPost()) {
            return $this->view('admin/login', array('error' => Session::pull('login_error', '')));
        }
        ...
        if (!RateLimiter::attempt('login:ip', 10, 900) || !RateLimiter::attempt('login:user', 6, 900, strtolower($username))) {
            ...
            Session::flash('error', 'Demasiados intentos. Vuelve a probar en ' . ceil($wait / 60) . ' minutos.');
            return $this->view('admin/login', array('error' => 'Demasiados intentos. Espera ' . ceil($wait / 60) . ' minutos.'));
        }

        $user = DB::first(
            'SELECT * FROM users WHERE (username = :u OR email = :e) AND is_active = 1 LIMIT 1',
            array('u' => $username, 'e' => $username)
        );

        if (!$user || !Security::verifyPassword($password, $user['password_hash'])) {
            Audit::log('login_failed', 'user', $user ? (int)$user['id'] : 0, array('username' => $username), 0);
        ...
        if ($user['restaurant_id']) {
            $status = DB::value('SELECT status FROM restaurants WHERE id = :r', array('r' => (int)$user['restaurant_id']));
            if ($status === 'suspended') {
                return $this->view('admin/login', array('error' => 'Este restaurante está suspendido. Contacta al administrador de la plataforma.'), 403);
            }
        }

        if (Security::needsRehash($user['password_hash'])) {
            DB::update('users', array('password_hash' => Security::hashPassword($password)), 'id = :id', array('id' => (int)$user['id']));
        }

        Auth::login($user);
        RateLimiter::clear('login:ip');
        RateLimiter::clear('login:user', strtolower($username));
        Audit::log('login', 'user', (int)$user['id'], array(), (int)$user['restaurant_id']);
    ...
    /** Acceso rápido con PIN para meseros en tablet compartida. */
    public function pin()
    {
        if (!$this->request->isPost()) {
            return $this->view('admin/pin');
        }
        $bad = $this->guardCsrf();
        if ($bad) { return $bad; }
        ...
        $candidates = DB::all("SELECT * FROM users WHERE is_active = 1 AND pin <> '' AND role IN ('waiter','kitchen','manager')");
        foreach ($candidates as $u) {
            if (Security::verifyPassword($pin, $u['pin'])) {
                Auth::login($u);
                Audit::log('login_pin', 'user', (int)$u['id'], array(), (int)$u['restaurant_id']);
