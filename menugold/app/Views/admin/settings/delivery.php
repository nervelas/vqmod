...
<form method="post" action="<?= e(mg_url('/panel/ajustes/entregas')) ?>">
  <?= Csrf::field() ?>
  <div class="card">
      ...
      foreach ($rows as $z): ?>
        <div class="grid grid-4" style="margin-bottom:.6rem;align-items:end">
          ...
          <div class="field" style="margin:0"><label>Pedido mínimo</label>
            <input class="input" name="zone_min[]" type="number" step="0.01" min="0" value="<?= e($z['min_order']) ?>"></div>
          <div class="field" style="margin:0"><label>Minutos</label>
            <input class="input" name="zone_eta[]" type="number" min="5" max="240" value="<?= e($z['eta_minutes']) ?>"></div>
        </div>
      <?php endforeach; ?>
    </div>
  ...
  <div class="card mt-2">
    ...
    <div class="row">
      <?php foreach ($zones as $z): ?>
        <form method="post" action="<?= e(mg_url('/panel/ajustes/entregas')) ?>" data-confirm="¿Eliminar la zona «<?= e($z['name']) ?>»?">
          <?= Csrf::field() ?>
