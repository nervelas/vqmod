  ...
  <div>
    ...
    <p class="cover-state reveal" style="--d:300ms">
      ...
      <span><?= e($state['open'] ? Lang::get('menu.open') : Lang::get('menu.closed')) ?><?php
        if ($state['open'] && $state['closes_at']) { echo ' · hasta ' . e($state['closes_at']); }
        elseif (!$state['open'] && $state['opens_at']) { echo ' · abre ' . e($state['opens_at']); }
      ?></span>
    </p>
...
<div class="sheet" id="menu-sheet">
  ...
  <?php if (!$state['open']): ?>
    <div style="padding:1rem var(--gutter) 0">
      <div class="alert">Ahora mismo estamos cerrados. Puedes ver el menú completo<?= $state['opens_at'] ? ' y volver a partir de las ' . e($state['opens_at']) : '' ?>.</div>
    </div>
  <?php endif; ?>
      ...
      <?php if (count($langs) > 1): ?>
        <div class="lang-switch">
          <?php foreach ($langs as $l): ?>
            <a href="<?= e(mg_url('/r/' . $r['slug'] . '/idioma/' . $l)) ?>" class="<?= Lang::locale() === $l ? 'is-on' : '' ?>"><?= e($l) ?></a>
          <?php endforeach; ?>
        </div>
      <?php endif; ?>
    </div>
