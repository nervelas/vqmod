...
$flow = array('new' => 0, 'preparing' => 1, 'ready' => 2, 'delivered' => 3, 'paid' => 3);
$current = isset($flow[$o['status']]) ? $flow[$o['status']] : 0;
$cancelled = $o['status'] === 'cancelled';
$steps = array(
    array(Lang::get('track.received'), $o['placed_at']),
    array(Lang::get('track.preparing'), $o['accepted_at']),
    array(Lang::get('track.ready'), $o['ready_at']),
    array(Lang::get('track.delivered'), $o['delivered_at']),
);
$showReview = isset($settings['review_prompt']) ? $settings['review_prompt'] !== '0' : true;
?>
<div class="track-wrap" data-track="<?= e($o['track_token']) ?>" data-status="<?= e($o['status']) ?>">
  ...
  <div class="card" style="background:linear-gradient(170deg,var(--carbon-2),var(--carbon));border:1px solid var(--line-soft);border-radius:var(--r-md);padding:1.3rem">
    <?php foreach ($o['items'] as $it): ?>
      <div class="cart-line" style="grid-template-columns:1fr auto">
        <div>
          <b><?= (int)$it['qty'] ?>× <?= e($it['name_snapshot']) ?></b>
  ...
  <?php if ($r['bank_info'] !== '' && $o['payment_status'] !== 'paid' && !$cancelled): ?>
    <div class="card mt-2" style="border:1px solid var(--line);border-radius:var(--r-md);padding:1.3rem">
      ...
      <?php if ($r['payment_url'] !== ''): ?>
        <a class="btn btn-block mt-2" href="<?= e($r['payment_url']) ?>" target="_blank" rel="noopener">Pagar en línea</a>
      <?php endif; ?>
    </div>
  <?php endif; ?>
  ...
  <?php if ($showReview && $r['review_url'] !== '' && in_array($o['status'], array('delivered', 'paid'), true)): ?>
  ...
  <p style="text-align:center;margin-top:2.5rem">
    <a class="link-line muted" href="<?= e(mg_url('/r/' . $r['slug'])) ?>">Volver al menú</a>
  </p>
</div>
