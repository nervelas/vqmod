...
try {
    $n = DB::run("UPDATE service_calls SET status='done', resolved_at=NOW()
...
try {
    $n = DB::run("UPDATE tables t SET t.status='free'
                  WHERE t.status <> 'free' AND NOT EXISTS (
                      SELECT 1 FROM orders o
                      WHERE o.table_id = t.id AND o.status IN ('new','preparing','ready','delivered'))")->rowCount();
    if ($n > 0) { $done[] = $n . ' mesas liberadas'; }
} catch (Throwable $e) { Logger::error('cron tables: ' . $e->getMessage()); }

/* 4 · Restaurantes con plan vencido se suspenden */
try {
    $n = DB::run("UPDATE restaurants SET status='suspended'
                  WHERE status <> 'suspended' AND plan_expires_at IS NOT NULL AND plan_expires_at < CURDATE()")->rowCount();
    if ($n > 0) { $done[] = $n . ' restaurantes suspendidos por vencimiento'; }
} catch (Throwable $e) { Logger::error('cron plans: ' . $e->getMessage()); }

/* 5 · Bitácora: se conservan 90 días */
try {
    $n = DB::run("DELETE FROM audit_log WHERE created_at < DATE_SUB(NOW(), INTERVAL 90 DAY)")->rowCount();
    if ($n > 0) { $done[] = $n . ' entradas de bitácora purgadas'; }
} catch (Throwable $e) { Logger::error('cron audit: ' . $e->getMessage()); }

/* 6 · Respaldo semanal automático */
try {
    $last = (int)Setting::get('last_backup_ts', '0');
    if (time() - $last > 7 * 86400) {
        $file = Backup::create(8);
        Setting::put('last_backup_ts', (string)time());
        $done[] = 'respaldo creado: ' . basename($file);
    }
} catch (Throwable $e) { Logger::error('cron backup: ' . $e->getMessage()); }

/* 7 · Archivos de sesión y caché antiguos */
try {
    $removed = 0;
...
/* 8 · Registros de más de tres meses */
try {
...
Setting::put('last_cron_ts', (string)time());
