  ...
  <div class="garita-lista">
    <?php foreach ($emergencia as $c): ?>
      <a class="garita-item" href="tel:<?= e(preg_replace('/\D+/', '', (string) $c['telefono'])) ?>" style="text-decoration:none;color:inherit">
        <span style="color:var(--acento-2)"><?= ico('telefono', 22) ?></span>
        <div class="crecer">
          <b><?= e($c['nombre']) ?></b>
          <small><?= e($c['tipo'] ?? '') ?></small>
        </div>
        <b style="color:var(--acento-2);font-size:1.1rem"><?= e($c['telefono']) ?></b>
      </a>
    <?php endforeach; ?>
  </div>
</section>
        ...
        <?php foreach ($casas as $c): ?>
          <tr>
            <td style="border:0;padding:9px 6px">
              <b style="color:var(--acento-2)"><?= e($c['codigo']) ?></b>
              <?php if ((int) $c['restringida'] === 1): ?><span class="chip grave">Restringida</span><?php endif; ?>
              <div style="color:rgba(233,238,233,.7);font-size:.82rem"><?= e(recortar((string) $c['residente'], 30) ?: 'Sin residente') ?></div>
            </td>
            <td style="border:0;padding:9px 6px;text-align:right">
              ...
              <?php else: ?><span style="color:rgba(233,238,233,.4)">Sin teléfono</span><?php endif; ?>
            </td>
          </tr>
        <?php endforeach; ?>
      </tbody>
